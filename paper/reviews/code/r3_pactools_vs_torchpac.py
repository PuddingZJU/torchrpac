...
n_pha = 50
n_amp = 40
# method correspondance between pactools and torchpac
METH = dict(
        PACTOOLS=dict(MVL='canolty', MI='tort', PLV='penny'),
        TORCHPAC=dict(MVL=1, MI=2, PLV=5)
    )
###############################################################################
...
# construct frequency vectors that fit both torchpac and pactools
f_pha_pt_width = 1.
...
def compute_pt_tp(n_jobs, n_perm, title=''):
    """Compute Pactools and Torchpac."""
    cpt, meth_comp, soft = [], [], []
    for meth in ['MVL', 'MI', 'PLV']:
        # ---------------------------------------------------------------------
        # get the method name
        meth_pt = METH['PACTOOLS'][meth]
        meth_tp = METH['TORCHPAC'][meth]
...
plt.tight_layout()
plt.savefig(f"../figures/r3_pactools_vs_torchpac.png", dpi=300,
            bbox_inches='tight')
